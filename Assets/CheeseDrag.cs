...
public class CheeseDrag : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        // Start dragging the cheese object
        isDragging = true;
    }

    void OnMouseUp()
    {
        // Stop dragging the cheese object
        isDragging = false;

        // Check if released over the pizza stone
        if (pizzaStoneCollider != null && pizzaStoneCollider.OverlapPoint(transform.position))
        {
            Debug.Log("Cheese placed on pizza stone. Transitioning to the CheesePlaced scene.");
            SceneManager.LoadScene("CheesePlaced"); // Change to your scene's name
        }
    }

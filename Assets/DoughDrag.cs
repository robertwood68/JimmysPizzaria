...
public class DoughDrag : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        // Start dragging the dough ball
        isDragging = true;
    }

    void OnMouseUp()
    {
        // Stop dragging the dough ball
        isDragging = false;

        // Check if released over the pizza stone
        if (pizzaStoneCollider != null && pizzaStoneCollider.OverlapPoint(transform.position))
        {
            Debug.Log("Dough placed on pizza stone. Transitioning to the next scene.");
            SceneManager.LoadScene("DoughPlaced"); // Change to your scene's name
        }
    }

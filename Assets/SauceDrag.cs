...
public class SauceDrag : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        // Start dragging the sauce object
        isDragging = true;
    }

    void OnMouseUp()
    {
        // Stop dragging the sauce object
        isDragging = false;

        // Check if released over the pizza stone
        if (pizzaStoneCollider != null && pizzaStoneCollider.OverlapPoint(transform.position))
        {
            Debug.Log("Sauce placed on pizza stone. Transitioning to the next scene.");
            SceneManager.LoadScene("SaucePlaced"); // Change to your scene's name
        }
    }
